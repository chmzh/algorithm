...
public class HuffmanCompressor {
	...
	private PriorityQueue<BinaryTree<CharFreq>> minPQ = new PriorityQueue<BinaryTree<CharFreq>>(
			11, new TreeComparator());
	private Map<Character, String> charCodes = new HashMap<Character, String>();
	private String inputPath = getFilePath();
	private String outputPath = inputPath.substring(0,
			inputPath.indexOf(".txt"));
	...
	/**
	 * @author Vanny Nguyen Frequency map for compression
	 * @throws IOException
	 */
	...
	/**
	 * @author Vanny Nguyen create CharFreq trees and insert in minPQ
	 */
	...
	/**
	 * @author Vanny Nguyen combine BinaryTrees within minPQ
	 */
	...
	public void makeSingleTree() throws IOException {
		...
		while (minPQ.size() > 1) {
			...
			minPQ.remove();
			BinaryTree<CharFreq> merged = new BinaryTree<CharFreq>(
					new CharFreq(left.getValue().getFreq()
							+ right.getValue().getFreq()), left, right);
			minPQ.add(merged);
		}
	}
	 ...
	 * @author Vanny Nguyen
	 * @param BinaryTree
	 *            to traverse, String code
	 */
	public void makeCharCodes(BinaryTree<CharFreq> cTree, String code)
			throws IOException {
	...
	public void compress() throws IOException {
		...
		if (minPQ.peek() != null) {
			...
			BufferedReader inputFile = new BufferedReader(new FileReader(
					inputPath));
			BufferedBitWriter outputCode = new BufferedBitWriter(outputPath
					+ "-compressed.txt");
			try {
	...
	public void decompress() throws IOException {
		BufferedBitReader inputCode = new BufferedBitReader(outputPath
				+ "-compressed.txt");
